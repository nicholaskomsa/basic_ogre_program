...
Ogre::Matrix4 Camera::getViewProj() {
	if (mCamera == nullptr)
		OGRE_EXCEPT(Ogre::Exception::ERR_RT_ASSERTION_FAILED, "mCamera is nullptr", "MapCamera::getViewProj");
...
Ogre::SceneNode* Camera::getSceneNode() {
	if (mCameraNode != nullptr)
		return mCameraNode;
	EXCEPT(Exception) << "Camera::getSceneNode mCameraNode is nullptr";
}
